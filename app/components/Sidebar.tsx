...
export function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <aside className="fixed right-0 top-0 z-50 h-full w-72 bg-[#141D32] border-l border-white/5 shadow-2xl">
        <div className="flex justify-between items-center p-4 border-b border-white/10">
          <span className="text-[#D4AF37] font-semibold tracking-wide">Menu</span>
        ...
        <nav className="p-3 space-y-0.5">
          {menuItems.map((item) => (
            <Link
              ...
              href={item.href}
              onClick={onClose}
              className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-white/90 hover:text-white hover:bg-white/10 transition-all duration-150"
            >
              <item.Icon />
          ...
          <div className="pt-3 mt-3 border-t border-white/10">
            <p className="px-3 py-2 text-[#94a3b8] text-xs font-semibold uppercase tracking-wider">Ajuda</p>
            <Link
              href="/suporte"
              onClick={onClose}
              className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-white/90 hover:text-white hover:bg-white/10 transition-all duration-150"
          ...
          <Link
            href="/login"
            onClick={onClose}
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-red-400/90 hover:text-red-400 hover:bg-red-500/10 transition-all duration-150 mt-2"
          >
            <LogoutIcon />
